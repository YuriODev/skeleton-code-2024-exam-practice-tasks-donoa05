...
class Puzzle():
  ...
  def __init__(self, *args):
    if len(
        args
    ) == 1:  #this will happen when one of the puzzle text files Are chosen
      self.__Score = 0
...
class Cell():
  ...
  def __init__(self):
    ...
    ]  #once a 3*3 grid is completed, the other letter connot be added to the at 3*3 grid and are added to this array
  ...
  def UpdateCell(
      self
  ):  #could ask you to allow the user to change the symbol in the cell to one of the previous symbols
    pass
