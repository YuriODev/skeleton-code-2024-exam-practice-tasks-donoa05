...
def Main():
  ...
  while Again == "y":
    ...
    if len(Filename) > 0:
      ...
      MyPuzzle = Puzzle(8, int(
          8 * 8 *
          0.6))  #It only lets the user fill in 60% of the grid which is 38.
...
class Puzzle():
  ...
  def __init__(self, *args):
    if len(args) == 1:#this will happen when one of the puzzle text files Are chosen
      self.__Score = 0
      self.__SymbolsLeft = 0 #the number of symbols that can be placed is determined by the MyPuzzle second value.
      self.__GridSize = 0
      self.__Grid = []
      self.__AllowedPatterns = []
      self.__AllowedSymbols = []
      self.__LoadPuzzle(args[0])
    else:  #this will happen when a random 8x8 grid is chosen, as 2 arguments are given
      self.__Score = 0
      ...
      for Count in range(1, self.__GridSize * self.__GridSize + 1):
        if random.randrange(
            1, 101) < 90:  #chooses random number when between 1 and 101
          C = Cell()  #90 percent chance of not a blank cell
        else:
          C = BlockedCell()  #10% chance of the cell being blocked
        self.__Grid.append(C)
      self.__AllowedPatterns = []
      self.__AllowedSymbols = []
      #these patterns are checked in a spiral in a 3*3 grid
  ...
  def __LoadPuzzle(self, Filename):
    try:
      with open(Filename) as f:
        ...
        self.__SymbolsLeft = int(f.readline().rstrip()) #the last line in the puzzle files are the number of attempts left for the symbols to be placed.
    except:
  ...
  def AttemptPuzzle(self):
    ...
    while not Finished:
      ...
      #=============================================================
      print(f"{self.__SymbolsLeft} symbol left")
      ...
      if RemoveSymbol:
        if CurrentCell.IsEmpty():
          print("Cell is empty")
        elif CurrentCell.CheckSymbolAllowed(CurrentCell.GetSymbol()) is False:
          print("Symbol not allowed")
        elif CurrentCell.GetSymbol() == "@":
          print("Cannot overide blocked cell")
        else:
...
class Cell():
  ...
  def __init__(self):
    self._Symbol = ""
    self.__SymbolsNotAllowed = []#once a 3*3 grid is completed, the other letter connot be added to the at 3*3 grid and are added to this array
  ...
  def UpdateCell(self):#could ask you to allow the user to change the symbol in the cell to one of the previous symbols
    pass
